...
******* MAPTYPE.H ************** Железняков В.A. ****** 07/02/12 ***
*                                                                  *
...
typedef __int64   HVECT;   // (УКАЗАТЕЛЬ НА TVectorImageEdit)       // 29/07/03
...
typedef __int64  HFORMULA;     // УКАЗАТЕЛЬ НА TStrFormula          // 17/01/07
...
typedef long int HIMAGE;       // ИДЕНТИФИКАТОР TCopyImage

typedef void * HMAPREG;        // ИДЕНТИФИКАТОР СПИСКА ПАРАМЕТРОВ СИСТЕМ ОТСЧЕТА

typedef HANDLE HWMS;           // ИДЕНТИФИКАТОР WMS-СЕРВИСА
...
#define PF_SKIPSEMCOLOR  8  // Флаг пропуска цвета по семантике  // 11/08/10
...
enum  MAPFILES                // ИДЕНТИФИКАТОРЫ ФАЙЛОВ (Intel)
{
    ...
    FILE_DGN  = 0x004E4744,   // ФАЙЛ Macrostation(DGN)   // 23/03/04
    ...
    FILE_DBF  = 0x00464244,   // ФАЙЛ DBF (База данных)             // 16/12/04
    FILE_TIN  = 0x004E4954,   // ФАЙЛ TIN                           // 31/03/05
    FILE_XLS  = 0x0000CFD0,   // ФАЙЛ EXCEL                         // 22/07/09
    FILE_WMS  = 0x00534D57,   // URL для подключения к WMS-сервису  // 16/07/11
...
enum PRECISIONFLAG              // ТОЧНОСТЬ ХРАНЕНИЯ КООРДИНАТ НА КАРТЕ
{
  ...
  PF_GEO    = 4                 // Повышенная точность в радианах     // 17/03/10
};


enum TMCMODE               // РЕЖИМЫ ДОСТУПА К ДАННЫМ                 // 10/05/11  
{
...
// ФОРМАТ ЗАПОЛНЯЕМОГО ЭЛЕМЕНТА DIB
  ...
  #define CELLSIZE 4
    ...
    #define CELLSIZE 1
  #else
    typedef CELL4  CELL;  // 14/12/09 Для Linux по умолчанию 4 байта
    #define CELLSIZE 4
  #endif
 #endif  // NUMBERTURN
#else
...
{           // 11/10/02
   ...
   unsigned int Italic     : 1;  // наклон символов    // 13/06/06
...
typedef struct TMCMAPLIST   // СПИСОК ДОСТУПНЫХ КАРТ
{
  ...
  int Ident;           // Идентификатора записи 0x7F7F7F7F                         // 26/03/12
...
enum OBJECT_DIRECT
{
   ...
   OD_RIGHT     = 2,     // Объект справа от контура 
   OD_LEFT      = 4,     // Объект слева от контура
   OD_CLOCKWISE = 5,     // Все части контура объекта цифруются по часовой стрелке  //23/01/12
   OD_ANTICLOCKWISE = 6, // Все части контура объекта цифруются против часовой стрелки  
};
...
#define EXTSEMANTICEND     31007  // КОНЕЦ ДИАПАЗОНА
...
#define SEMADRPOSTCODE     31209  // ПОЧТОВЫЙ ИНДЕКС          // 12/03/09
...
#define SEMOBJECTTOTEXT     32804  // ССЫЛКА НА ПОДПИСЬ ОТ ОБ'ЕКТА
                                   // (НОМЕР ОСНОВНОГО ОБЪЕКТА)
#define SEMOBJECTFROMTEXT   32805  // ССЫЛКА НА ОБ'ЕКТ ОТ ПОДПИСИ
                                   // (НОМЕР ОБЪЕКТА-ПОДПИСЬ)

#define SEMRSCNAME          32809  // ИМЯ ФАЙЛА КЛАССИФИКАТОРА RSC   // 16/02/10
#define SEMLAYERSHORTNAME   32810  // КОРОТКОЕ ИМЯ СЛОЯ ОБ'ЕКТА
#define SEMOBJECTSHORTNAME  32811  // КОРОТКОЕ ИМЯ ОБ'ЕКТА (КЛЮЧ)
#define SEMOBJECTIDENT      32812  // ИДЕНТИФИКАТОР ОБЪЕКТА     // 13/01/12


// СЛУЖЕБНЫЕ СЕМАНТИКИ НЕ РЕДАКТИРУЮТСЯ !
#define SEMHIDEFIRST        32800  // НАЧАЛО ДИАПАЗОНА СЛУЖЕБНЫХ СЕМАНТИК
#define SEMHIDELAST         32800  // КОНЕЦ  ДИАПАЗОНА СЛУЖЕБНЫХ СЕМАНТИК  // 02/08/11
...
#define SEMOBJECTDATE       32850  // ДАТА СОЗДАНИЯ ОБ'ЕКТА
#define SEMOBJECTTIME       32851  // ВРЕМЯ СОЗДАНИЯ ОБ'ЕКТА
#define SEMOBJECTAUTHOR     32852  // ИМЯ ОПЕРАТОРА
#define SEMOBJECTREDATE     32853  // ДАТА ОБНОВЛЕНИЯ ОБ'ЕКТА        // 17/09/09
#define SEMOBJECTRETIME     32854  // ВРЕМЯ ОБНОВЛЕНИЯ ОБ'ЕКТА
...
// Запрет перехода для случая когда, ребра соединяются в двух узлах - в начале и в конце         // 18/10/10
...
typedef struct BUILDMTW
{
  ...
  long int Unit;           // Eдиница измерения высоты матрицы высот MTW
                           // (0 - метры, 1 - дециметры,
                           //  2 - сантиметры, 3 - миллиметры )

  long int ReliefType;     // Тип матрицы высот MTW (0 - абсолютные высоты,
                           //  1 - абсолютные + относительные,
                           //  2 - относительные)
 ...
 long int Scale;          // Знаменатель масштаба создаваемой
  ...
  long int HeightSuper;    // Флаг занесения высоты в элемент при создании матрицы
 ...
 long int Method;    // Метод построения поверхности при создании матрицы высот MTW
...
{                                 //   (ДАННЫЕ О ПРОЕКЦИИ)
  ...
  long      ZoneNumber;         // Номер зоны топокарты                   // 28/11/07
  ...
  long      ZoneIdent;       // Идентификатор района (для МСК 63: A-X или 0)     // 26/04/11
...
typedef struct BUILDSURFACE  // 15/06/11
{
 ...
 long int SearchSectorCount; // Количество секторов для поиска соседних точек (для Method = 9,11) // 09/06/11
                             // 1 - по кругу
                             // 4 - по 4 секторам
                             // 8 - по 8 секторам
 ...
 long int PointCount;    // Число точек в массиве PointArray (для Method = 8)

 long int SemanticCode;  // Код семантики моделируемой характеристики (для Method = 9)

 long int LocalSurfacePointCount;  // Kоличество точек для построения локальной поверхности
                                   // вокруг определяемого элемента (для Method = 9,11)
 ...
 long int LocalSurfaceRebuildPointCount;  // Kоличество обновленных точек при переходе
                                          // к очередному определяемому элементу,
                                          // при котором перестраивается локальная
                                          // поверхность (для Method = 9)
 double MaxMetricCutLength;  // Шаг добавления точек по контуру линейных и площадных объектов (для Method = 9,11)
                             // Используется если IsMetricCutLength = 1

 long int Use3DMetric;       // Флаг использования высот из трехмерной метрики объектов :  // 06/11/07
                             //  0 - высоты из трехмерной метрики не используются
                             //  1 - высоты из трехмерной метрики используются

                             // Флаг использования параметра Border        24/11/09
                             // Если Border - идентификатор площадного объекта с подобъектами,то
                             // при BuildInsideBorderSubjects = 0 область заполняемых элементов матрицы
                             // определяется заданным площадным объектом Border с учётом подобъектов,
                             // при BuildInsideBorderSubjects = 1 область заполняемых элементов
                             // матрицы определяется подобъектами площадного объекта Border.
                             // Если Border - идентификатор линейного объекта или площадного объекта
                             // без подобъектов, то флаг BuildInsideBorderSubjects устанавливается в ноль.
                             // (Данный флаг задаёт режим "Строить только внутри подобъектов" и позволяет
                             //  построить матрицу, содержащую несколько отдельных заполненных областей).
 long int BuildInsideBorderSubjects;

                             // Флаг использования параметра Border        24/12/09
                             // Если Border - идентификатор площадного объекта,то
                             // при BuildInsideBorderMainObject = 0 область заполняемых элементов матрицы
                             // определяется заданным площадным объектом Border с учётом подобъектов,
                             // при BuildInsideBorderMainObject = 1 область заполняемых элементов
                             // матрицы определяется главным объектом площадного объекта Border.
                             // Если Border - идентификатор линейного объекта, то флаг
                             // BuildInsideBorderMainObject устанавливается в ноль.
                             // (Данный флаг задаёт режим "Строить внутри всего главного контура" и позволяет
                             //  построить матрицу с прогнозируемыми значениями в областях отсутствия данных).
 long int BuildInsideBorderMainObject;

 long int IsMetricCutLength; // Добавлять точки по контуру линейных и площадных объектов (для Method = 9,11) // 09/06/11
                             // Точки добавляются с шагом MaxMetricCutLength

 long int IsAddPointsInEmptyRegion; // Добавлять точки в пустых областях (для Method = 9,11) // 09/06/11

 long int IsLimitHeight; // Если включено, то высота элемента матрицы не может быть больше или меньше высот (для Method = 9,11) // 09/06/11
                         // соседних точек, по которым строится поверхность, на LimitOffset

 double   DistBeforePointsInEmptyRegion; // Расстояние между точками в пустых областях (для Method = 9,11) // 09/06/11
 ...
 double   LimitOffset; // Если включено IsLimitHeight то разрешенное отклонение вычисленного  (для Method = 9,11) // 09/06/11
...
enum ACTION_TYPE
{
  ...
  AT_LAST     = AT_REDO,   // Последний зарезервированный код
};
...
typedef struct MEDPROPERTY
{
 ...
 double TakeRadius;             // Радиус захвата точек и линий //23/03/06
 ...
 long int IsPanel;              // Вспомогательные панели  // 03/10/06

 long int MapSplineType;        // Тип сплайна для MAP   //11/08/03
 long int SitSplineType;        // Тип сплайна для SIT   //11/08/03

 long int Modelcount;           // Количество макетов    //03/12/04
 long int MeasureUnit;          // Единица измерения     // 29/09/05

 long int BigButton;            // Размер кнопок         //17/01/08
 long int IsDateTime;           // Писать время и дату создания/редактирования //21/09/2009

// Параметры насыпи

 double   Exc_Step;             // Расстояние между штрихами (мм)
 double   Exc_Lenght;           // Длина кор. штр. в (мм)
 double   Exc_MaxWidth;         // Макс. ширина для фикс.размера в (мм)

 long int Exc_Proport;          // Установить пропорциональную длину?
 long int Exc_Numerator;        // Числитель пропорции                     //06/05/11
 long int Exc_Denominator;      // Знаменатель пропорции
 long int Exc_Pitching;         // Укрепленные?


 long int MoveMaxCount;         // Количество перерисовываемых при перемещении
 long int MoveViewType;         // 0 - все, 1- первые, 2 - случайно
...
typedef struct OLELOADPARM
{
 ...
 long int Height;               // высота Ole-объекта в мм на карте в единичном масштабе
 long int Width;                // ширина Ole-объекта в мм на карте в единичном масштабе
