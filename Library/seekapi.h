/****** SEEKAPI.H ************** Belenkov O.V.   ****** 08/04/12 ***
******* SEEKAPI.H ************** Nefedieva O.A.  ****** 07/05/08 ***
******* SEEKAPI.H ************** Tarakanova I.R. ****** 25/08/10 ***
...
_MAPIMP long int _MAPAPI mapSeekSelectObjectCount(HMAP hMap, HSELECT select);
 ...
 // Поиск ближайшего объекта по заданным условиям среди всех объектов   // 12/08/10
 // hMap     - идентификатор открытой карты,
 // info     - идентификатор существующего объекта,
 //            в котором будет размещен результат поиска;
...
_MAPIMP long int _MAPAPI mapTotalSeekObjectCount(HMAP hMap);
 ...
 // Подсчитать сколько объектов удовлетворяет условиям
 // обобщенного поиска на заданной карте
 // hMap  - идентификатор открытых данных
...
_MAPIMP  HSELECT _MAPAPI mapCreateRscSelectContext(HRSC hrsc);       // 07/05/10
...
_MAPIMP long int _MAPAPI mapCopySelectContextEx(HSELECT target, HSELECT source);   // 04/02/10
...
_MAPIMP long int _MAPAPI mapClearSelectContext(HSELECT select);
  ...
  // Установить доступ ко всем видам данных                // 01/08/08
...
_MAPIMP  long int _MAPAPI mapGetInversionSelect(HSELECT select);
  ...
  // Установить пересечение условий поиска (target = target & source)  // 02/11/09
...
_MAPIMP  long int _MAPAPI mapSelectAndSelect(HSELECT target, HSELECT source);
  ...
  // Установить пересечение условий поиска с операцией OR (target = target | source)   // 25/05/10
  // При выполнении операции учитываются только коды объектов,
...
_MAPIMP long int _MAPAPI mapIsSelectActive(HSELECT hselect);   // 09/04/08
...
_MAPIMP  long int _MAPAPI mapSetSampleAllObjects(HSELECT select, long int list);
  ...
  // заданный внутренний код                                          // 02/05/11
...
_MAPIMP  long int _MAPAPI mapSelectSeekAreaFrame(HSELECT hselect, DFRAME * dframe,
                                     double distance = 0.0,
                                     long int filter = 0,
                                     long int inside = 1,
                                     long int visible = 0,
...
_MAPIMP long int _MAPAPI mapGetAreaSelectFlag(HSELECT hselect);   //07/04/08
...
_MAPIMP long int _MAPAPI mapGetDrawObjectsFlag(HSELECT hselect);   //07/04/08
...
_MAPIMP void _MAPAPI mapSetDrawObjectsFlag(HSELECT hselect,long int flag);   //07/04/08
...
_MAPIMP int _MAPAPI mapCheckInsideObjectPoint(HOBJ info1, HOBJ info2,
                                              DOUBLEPOINT * point);
