/****** MAPGDI.H *************** Kruzhkov A.E.   ****** 29/10/09 ***
******* MAPGDI.H *************** Belenkov O.V.   ****** 18/04/12 ***
******* MAPGDI.H *************** Panteleeva N.A. ****** 06/08/10 ***
...
#define IMG_SQUAREGLASSCOLOR  164  // Полигон-цветное стекло
...
#define IMGTEXT_SHORTCLS 4                       // 12/12/11
...
{                                  // (size = 312)
  ...
  char     Name[256];              // Имя файла изображения
       ...
       int Height   ;              // Габариты знака (высота, ширина)
       int Width    ;              //
       int PosV     ;              // Точка привязки знака по горизонтали
       int PosH     ;              // и вертикали (от начала габаритов знака)
       int Shift        ;          // Смещение тени
  IMGLINE  Contour      ;          // Параметры контура
  char     ContourFlag  ;          // Флаг наличия контура
  char     ShadeFlag    ;          // Флаг наличия тени
  char     TransparentFlag;        // Флаг прозрачности фонового цвета     // 29/10/09
                                   // (первый пиксел картинки - левый нижний угол BMP)
  char     Reserve[13]  ;          // Резерв (должен быть 0)
}
 IMGGRAPHICMARK;
